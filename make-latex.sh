...
for filename in text/ch*.txt; do
   [ -e "$filename" ] || continue
   pandoc --lua-filter=extras.lua "$filename" --to markdown | pandoc --lua-filter=extras.lua --to markdown | pandoc --lua-filter=info.lua --to markdown | pandoc --lua-filter=epigraph.lua --to markdown | pandoc --lua-filter=figure.lua --to markdown | pandoc --lua-filter=footnote.lua --to markdown | pandoc --filter pandoc-fignos --to markdown | pandoc --metadata-file=meta.yml --top-level-division=chapter --citeproc --bibliography=bibliography/"$(basename "$filename" .txt).bib" --reference-location=section --wrap=none --to latex > latex/"$(basename "$filename" .txt).tex"
done
...
pandoc -N --quiet --variable "geometry=margin=1.2in" --variable mainfont="Sans Regular" --variable sansfont="Sans Regular" --variable monofont="Sans Regular" --variable fontsize=9pt --variable version=2.0 book.tex  --pdf-engine=xelatex --toc -o book.pdf*

pandoc --quiet -f latex book.tex -o book.epud
